...
namespace Ecommerce.Application.Services.Implementation
{
    public class DashboardService : IDashboardService
    {
        ...
        {
            var totalBookings = _unitOfWork.Booking.GetAll(b => b.BookingDate >= DateTime.Now.AddDays(-30) && b.status != SD.Status_Pending || b.status == SD.Status_Cancelled);
        ...
        public async Task<LineChartDto> GetMemberAndBookingLineChartData()
        {
            ...
            var customerData = _unitOfWork.User.GetAll(u => u.CreatedOn >= DateTime.Now.AddDays(-30) && u.CreatedOn.Date <= DateTime.Now)
                .GroupBy(b => b.CreatedOn.Date).Select(u => new {
                    DateTime = u.Key,
        ...
        public async Task<RadioBarChartDto> GetRegisteredUserChartData()
        {
            var totaUsers = _unitOfWork.User.GetAll();
            var countByCurrentMonth = totaUsers.Count(u => u.CreatedOn >= currentMonthStartDate && u.CreatedOn <= DateTime.Now);
            var countByPreviousMonth = totaUsers.Count(u => u.CreatedOn >= previousMonthStartDate && u.CreatedOn <= currentMonthStartDate);

            return SD.GetRadialChartDataModel(totaUsers.Count(), countByCurrentMonth, countByPreviousMonth);
        }
        public async Task<RadioBarChartDto> GetRevenueChartData()
        {
            var totalBookings = _unitOfWork.Booking.GetAll(b => b.status != SD.Status_Pending || b.status == SD.Status_Cancelled);
        ...
        public async Task<RadioBarChartDto> GetTotalBookingRadialChartData()
        {
            var totalBookings = _unitOfWork.Booking.GetAll(b => b.status != SD.Status_Pending || b.status == SD.Status_Cancelled);
